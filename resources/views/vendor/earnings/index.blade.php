...
<style>
    ...
    .vd-kpi-sub { font-size:11px;color:var(--cb-muted);line-height:1.6; }
    ...
    /* Reversement form */
    .vd-input,.vd-select { background:var(--cb-paper) !important;border:1px solid var(--cb-border) !important;color:var(--cb-ink) !important;border-radius:3px !important;font-family:'DM Sans',sans-serif !important;font-size:13px !important;padding:10px 14px !important;width:100%;outline:none;transition:border-color .25s; }
    .vd-input:focus,.vd-select:focus { border-color:rgba(176,134,46,.4) !important;box-shadow:0 0 0 3px rgba(176,134,46,.07) !important; }
    .vd-input::placeholder { color:var(--cb-muted) !important; }
    ...
    .vd-help { font-size:12px;color:var(--cb-muted);margin-top:5px; }

    /* min badge */
    .vd-min-badge { font-family:'Syne',sans-serif;font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;padding:4px 10px;border-radius:100px;background:rgba(15,92,67,.04);color:var(--cb-muted);border:1px solid var(--cb-border); }

    /* Locks box */
    .vd-locked-box { background:rgba(176,134,46,.04);border:1px solid rgba(176,134,46,.12);border-radius:3px;padding:12px 16px;font-size:13px;color:var(--cb-muted);line-height:1.7; }
    .vd-locked-box strong { color:var(--cb-gold); }

    /* Status note */
    .vd-status-ok  { font-size:13px;color:var(--cb-forest); }
    .vd-status-no  { font-size:13px;color:var(--cb-down); }
    ...
    .vd-table td:first-child { color:var(--cb-ink); }
    ...
    .po-pending  { background:rgba(176,134,46,.08);color:var(--cb-gold);border:1px solid rgba(176,134,46,.2); }
    .po-approved { background:rgba(43,127,196,.08);color:#2B7FC4;border:1px solid rgba(43,127,196,.2); }
    .po-paid     { background:rgba(15,92,67,.08);color:var(--cb-forest);border:1px solid rgba(15,92,67,.2); }
    .po-rejected { background:rgba(192,57,43,.08);color:var(--cb-down);border:1px solid rgba(192,57,43,.2); }
    ...
    .vd-alert-err { background:rgba(192,57,43,.06);border:1px solid rgba(192,57,43,.2);border-radius:3px;padding:12px 16px;font-size:13px;color:var(--cb-down);margin-bottom:16px; }
...
<div class="vd-page">
        ...
        <div class="container" style="max-width:1100px;">
            <div class="d-flex justify-content-between align-items-start flex-wrap gap-3">
                <div>
                    ...
                    <p class="vd-hero-sub">Commission Boursiv : <strong style="color:var(--cb-down);">15%</strong> · Délai sécurité : <strong style="color:var(--cb-gold);">{{ $delayH }}h</strong></p>
                </div>
    ...
    <div class="container py-5" style="max-width:1100px;">
        ...
        <div class="vd-kpi-grid cbr">
            <div class="vd-kpi" style="--kc:var(--cb-gold);">
            ...
            <div class="vd-kpi" style="--kc:var(--cb-down);">
            ...
            <div class="vd-kpi" style="--kc:var(--cb-forest);">
            ...
            <div class="vd-section-body">
                ...
                <form method="POST" action="{{ route('vendor.payouts.request') }}"
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="vd-help">Retirable : <strong style="color:var(--cb-ink);">{{ number_format($available,0,',',' ') }} FCFA</strong></div>
                    </div>
                ...
                <table class="vd-table">
                    ...
                    <tbody>
                        @forelse($payouts as $p)
                            ...
                            @endphp
                            <tr>
                                <td style="font-size:12px;color:var(--cb-muted);">{{ optional($p->requested_at ?? $p->created_at)->format('d/m/Y H:i') }}</td>
                                <td><span class="vd-price">{{ number_format((int)$p->amount,0,',',' ') }}</span> <span style="font-size:11px;color:var(--cb-muted);">F</span></td>
                                <td style="font-size:12px;color:var(--cb-muted);">{{ strtoupper((string)$p->payout_method) }}</td>
                                <td style="font-size:12px;color:var(--cb-muted);">{{ $p->payout_account }}</td>
